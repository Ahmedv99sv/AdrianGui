...
pcall(function()
	StarterGui:SetCore("SendNotification", {
		Title = "ðŸ“¢ Adrian Scripts Notice",
...
-- AdrianHub ðŸ¥š Egg ESP + Pet Replace (Visual Only) + RGG Jackpot Effect ðŸŽ‰
...
-- ðŸ’Ž Pets with RGG Jackpot Effect
...
-- âœ… Fixed: Only scan egg models once
...
local function jackpotNotify()
	...
	label.Text = "ðŸŽ‰ JACKPOT ðŸ¦ðŸ†ðŸŽ‰"
...
local function startCooldown(button, timer)
	...
	while currentCountdown > 0 do
		timer.Text = "â³ Wait " .. currentCountdown .. " seconds to change the pet inside the egg"
		for _, obj in ipairs(Workspace:GetDescendants()) do
			if obj:IsA("Model") and PetLists[obj.Name] then
				local part = obj:FindFirstChildWhichIsA("BasePart")
				if part then
	...
	timer.Text = "âœ… You can now change the pet inside the egg"
	for _, obj in ipairs(Workspace:GetDescendants()) do
		if obj:IsA("Model") and PetLists[obj.Name] then
			local part = obj:FindFirstChildWhichIsA("BasePart")
			if part then
...
local function createUI()
	...
	timerLabel.Text = "âœ… You can now change the pet inside the egg"
	...
	button.TextSize = 16
	button.Text = "ðŸ” Replace Pet Inside Egg"
...
toggleButton.Text = "ðŸ” Pet Detector"
...
-- SKIDDED BY SIGMA @rizzify101
-- SKIDDED BY SIGMA @rizzify101
-- SKIDDED BY SIGMA @rizzify101-- SKIDDED BY SIGMA @rizzify101
-- SKIDDED BY SIGMA @rizzify101
-- SKIDDED BY SIGMA @rizzify101
-- SKIDDED BY SIGMA @rizzify101
-- SKIDDED BY SIGMA @rizzify101
